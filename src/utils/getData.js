        ...
        axios.get('http://api.tianapi.com/star/index', {
            ...
            if (res.data.code == 200) {
                ...
                res.data.newslist.forEach((item, index) => {
                    ...
                    if (index % 2) {
                        str += `${item.type}：${item.content}\n`;
                    } else {
                        if (index == 6) {
                            str += `${item.type}：${item.content < 10 ? item.content + '    ' : item.content}  `;
                        } else {
                            str += `${item.type}：${item.content}  `;
                        }
                    }
...
const countDown = (time) => {
    ...
    const getTime = (time) => {
        ...
        return { FullYear, Month, Dates, Hours, Minutes, Seconds, date };
    };

    let { FullYear, Month, Dates, Hours, Minutes, Seconds, date } = getTime();

    let _FullYear = getTime(time).FullYear;
    let _Month = getTime(time).Month;
    let _Dates = getTime(time).Dates;
    let _Hours = getTime(time).Hours;
    let _Minutes = getTime(time).Minutes;
    let _Seconds = getTime(time).Month;
    let _date = getTime(time).date;

    let isadd = false;
    //判断设置日期是否过去
    if (Month > _Month) {
        isadd = true;
        time = `${FullYear + 1}-${_Month}-${_Dates}`;

        _FullYear = getTime(time).FullYear;
        _Month = getTime(time).Month;
        _Dates = getTime(time).Dates;
        _Hours = getTime(time).Hours;
        _Minutes = getTime(time).Minutes;
        _Seconds = getTime(time).Month;
        _date = getTime(time).date;

    } else if (Month == Month && Dates > _Dates) {
        isadd = true;
        time = `${FullYear + 1}-${_Month}-${_Dates}`;
...
countDown(config.data.birthday);


/**获取农历 start*/
function getLunar(data) {
    ...
    function NewTick(data = '') {
        noww = data ? new Date(data) : new Date();
